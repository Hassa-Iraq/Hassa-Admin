...
export async function GET(request) {
  try {
    ...
    const backendResponse = await axios.get(`${API_BASE_URL}/api/auth/admin/employees${search}`, {
      headers: {
...
export async function POST(request) {
  try {
    ...
    const backendResponse = await axios.post(
      `${API_BASE_URL}/api/auth/admin/employees`,
      payload,
      {
        headers: {
          ...(authHeader ? { Authorization: authHeader } : {}),
    ...
    return getErrorResponse(error, 'Failed to add employee');
  }
}
