...
export async function GET(request, context) {
  try {
    ...
    const backendResponse = await axios.get(`${API_BASE_URL}/api/auth/drivers/${driverUserId}`, {
      headers: {
    ...
    return getErrorResponse(error, 'Failed to fetch deliveryman details');
  }
}
...
export async function PATCH(request, context) {
  try {
    ...
    const backendResponse = await axios.patch(
      `${API_BASE_URL}/api/auth/drivers/${driverUserId}`,
      payload,
      {
        headers: {
          ...(authHeader ? { Authorization: authHeader } : {}),
    ...
    return getErrorResponse(error, 'Failed to update deliveryman');
  }
}
