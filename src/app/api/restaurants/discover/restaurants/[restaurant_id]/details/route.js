...
export async function GET(request, context) {
  try {
    ...
    const backendResponse = await axios.get(
      `${API_BASE_URL}/api/restaurants/discover/restaurants/${encodeURIComponent(restaurantId)}/details`,
      {
        headers: {
    ...
    if (axios.isAxiosError(error)) {
      ...
      const message =
        ...
        error.response?.data ||
        error.message ||
        'Failed to fetch restaurant discover details';

      return NextResponse.json({ message }, { status });
    }

    return NextResponse.json(
      { message: error?.message || 'Failed to fetch restaurant discover details' },
      { status: 500 }
    );
  }
}
