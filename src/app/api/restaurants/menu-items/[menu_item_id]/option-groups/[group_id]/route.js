...
export async function PATCH(request, context) {
  try {
    ...
    const backendResponse = await axios.patch(
      `${API_BASE_URL}/api/restaurants/menu-items/${menuItemId}/option-groups/${groupId}`,
      payload,
      {
        headers: {
    ...
    return getErrorResponse(error, 'Failed to update option group');
  }
}
...
export async function DELETE(request, context) {
  try {
    ...
    return getErrorResponse(error, 'Failed to delete option group');
  }
}
