  ...
  try {
    ...
    try {
      const backendResponse = await fetch(
        `${API_BASE_URL}/api/wallet/admin/payouts/${encodeURIComponent(String(payoutId))}/approve`,
        {
          method: 'POST',
          headers: {
    ...
    return NextResponse.json({ message: error?.message || 'Failed to approve payout' }, { status: 500 });
  }
}
