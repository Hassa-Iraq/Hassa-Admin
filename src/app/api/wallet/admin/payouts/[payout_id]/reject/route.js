  ...
  try {
    ...
    try {
      const backendResponse = await fetch(
        `${API_BASE_URL}/api/wallet/admin/payouts/${encodeURIComponent(String(payoutId))}/reject`,
        {
          method: 'POST',
          headers: {
    ...
    return NextResponse.json({ message: error?.message || 'Failed to reject payout' }, { status: 500 });
  }
}
