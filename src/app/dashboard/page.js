...
export default function Dashboard() {
  const { t } = useLanguage();
  const [orderStatsCards, setOrderStatsCards] = useState(() => statsTemplate.map((s) => ({ ...s, value: null })));
  ...
  useEffect(() => {
    ...
    const fetchOrderStats = async () => {
      try {
        ...
        if (!cancelled) {
          // Keep layout but avoid flashing template values.
          setOrderStatsCards(statsTemplate.map((s) => ({ ...s, value: 0 })));
        }
      } finally {
        if (!cancelled) setOrderStatsLoading(false);
      }
    };
      ...
      <div className="pt-32 md:pt-36 px-4 md:px-6 space-y-6 pb-16">
        ...
        >
          {orderStatsCards.map((s, i) => (
            <StatCard key={i} {...s} index={i} loading={orderStatsLoading} />
          ))}
        </OrderStats>
        <RevenueChart />
